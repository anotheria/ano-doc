...
public class EnumerationGenerator extends AbstractGenerator implements IGenerator{
	...
	private GeneratedClass generateDefinition(EnumerationType type){
		...
		for (int i=0; i<values.size(); i++){
			...
			appendStatement("static final int "+v+" = "+(i+1));
		}
		emptyline();
		
		for (String v : values){
			appendStatement("static final String "+v+"_NAME = "+quote(v));
		}

		emptyline();
		
		appendString("static final int "+type.getName().toUpperCase()+"_VALUES[] = {");
        for (String v : values) {
		...
		appendString("static final String "+type.getName().toUpperCase()+"_NAMES[] = {");
        for (String v : values) {
