...
public class AppUtilGenerator extends AbstractAnoDocGenerator{
	 ...
	 */
	public void generateFromModules(List<MetaModule> modules){
		...
		files.addAll(new StructureUtilsGenerator().generateModules(modules));
	 ...
	 */
	public void generateFromViews(List<MetaView> views){
		List<FileEntry> files = new ArrayList<FileEntry>();
		files.addAll(new StructureUtilsGenerator().generateViews(views));
